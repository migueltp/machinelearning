...
url_read = str(url_open.readlines())
zero = re.findall('0\_jackson\_[0-9]{1,2}\.wav+', url_read)
zero_df = pd.DataFrame(zero)

os.chdir('Datasets')    
url = 'https://github.com/Jakobovski/free-spoken-digit-dataset/raw/master/recordings/'
i = pd.unique(zero_df.iloc[:,0])[0]

for i in pd.unique(zero_df.iloc[:,0]):
    print "Starting:::"+i    
    file_name = i
    curr_url = str(url+i)
    u = urllib2.urlopen(curr_url)
    f = open(file_name, 'wb')
    f.write(u.read())
    f.close()      
    print "Finished:::"+i
...
#
lim = int(Provided_Portion * test.shape[0])+1
X_test = test[1:lim]
...
#
lim = int(Provided_Portion * train.shape[1])+1
X_train = train[:,1:lim]
y_train = train[:,lim:]
