...
def drawPlots(model, wintitle='Figure 1'):
  ...
  colors = [y_colors[i-1] for i in y_train]
...
#
X = pd.read_csv("../SVM/Datasets/wheat.data")
X = X.drop('id', axis=1)
print X[pd.isnull(X).any(axis=1)]
X = X.dropna(axis=0)

X.wheat_type.unique()
y = X.wheat_type.map({'kama':1, 'canadian':2, 'rosa':3}) 
...
benchmark(dt, 'Dec Tree')
drawPlots(dt, 'Dec Tree')
...
# Lab Question 2
# Keep dropping the max_depth of the decision tree down until it's 
# high-dimensionality score is less than KNeighbors, and then stop.
...
while score > target:
    ...
    score = benchmark(dt, 'Dec Tree')
    drawPlots(dt, 'Dec Tree')
    print "With Max Depth ", max_depth, " Score = ", score
