...
splitTrainData = function(data_obj, train, test, validation) {
  
  print('Splitting data to train, test and validation - Start')
  set.seed(1)
  
  population = seq(1, nrow(data_obj), 1)
  train_sample = sample(population, size = floor(nrow(data_obj) * train), replace = FALSE)
  ...
  validation_sample = sample(population, size = floor(nrow(data_obj) * validation), replace = FALSE)
